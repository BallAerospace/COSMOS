...
describe('DataViewer', () => {
  ...
  it('adds a raw packet to a new tab', () => {
    ...
    cy.wait(1000)
    cy.get('[data-test=add-packet-button]').click().wait(1000)
    cy.get('[data-test=start-button]').click().wait(1000)
    cy.wait(1000) // wait for the first packet to come in
    cy.get('[data-test=dump-component-text-area]').should('not.have.value', '')
  })

  xit('adds a decom packet to a new tab', () => {
    cy.get('[data-test=new-tab]').click().wait(1000)
  ...
  it('renames a tab', () => {
    cy.get('[data-test=new-tab]').click().wait(1000)
    cy.get('[data-test=tab]').rightclick().wait(1000)
    ...
    cy.get('[data-test=rename]').click().wait(1000)
    ...
    cy.get('[data-test=rename-tab-input]')
      .clear()
      .type('Cancel this')
    cy.get('[data-test=cancel-rename]').click().wait(1000)
    cy.get('.v-tab').should('contain', 'Testing tab name')
  })
  ...
  it('deletes a component and tab', () => {
    ...
    cy.get('[data-test=delete-packet]').click().wait(1000)
    cy.get('.v-window-item > .v-card > .v-card__title').should(
      'contain',
      'This tab is empty'
    )
  ...
  it('controls playback', () => {
    ...
    cy.get('[data-test=dump-component-text-area]')
      ...
      .then((val) => {
        ...
        cy.get('[data-test=stop-button]').click().wait(1000)
  ...
  xit('saves and loads the configuration', () => {
    ...
    cy.get('.v-toolbar').contains('File').click().wait(1000)
    cy.contains('Save Configuration').click().wait(1000)
    ...
    cy.get('.v-toolbar').contains('File').click().wait(1000)
    cy.contains('Open Configuration').click().wait(1000)
    cy.get('.v-dialog:visible').within(() => {
      // Try to click OK without anything selected
      cy.contains('Ok').click().wait(1000)
      cy.contains('Select a configuration')
      cy.contains(config).click().wait(1000)
      cy.contains('Ok').click().wait(1000)
    })
    // Verify we're back
    cy.get('.v-window-item > .v-card > .v-card__title').should(
