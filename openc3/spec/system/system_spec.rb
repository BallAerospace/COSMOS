...
module OpenC3
  xdescribe System do
    before(:all) do
      ...
      # Save system.txt
      @config_file = File.join(OpenC3::USERPATH, 'config', 'system', 'system.txt')
      FileUtils.mv @config_file, OpenC3::USERPATH

      # Create a dummy system.txt
      File.open(@config_file, 'w') { |file| file.puts "AUTO_DECLARE_TARGETS" }
      @config_targets = File.join(OpenC3::USERPATH, 'config', 'targets')
    end

    after(:all) do
      # Restore system.txt
      FileUtils.mv File.join(OpenC3::USERPATH, 'system.txt'),
                   File.join(OpenC3::USERPATH, 'config', 'system')
      System.class_eval('@@instance = nil')
    end
      ...
      context "initializing SYSTEM META" do
        skip "TODO: Does System still initialize SYSTEM META?" do
          ...
          it "initializes the SYSTEM META with no definitions" do
            ...
            expect(tlm.read("OPENC3_VERSION")).to_not be_nil
            ...
            expect(cmd.read("OPENC3_VERSION")).to eql tlm.read("OPENC3_VERSION")
          ...
          it "defaults badly defined TELEMETRY SYSTEM META" do
            ...
            file.puts "APPEND_ITEM OPENC3_VERSION 240 STRING"
            ...
            expect(tlm.read("CONFIG")).to_not be_nil
            expect(tlm.read("OPENC3_VERSION")).to_not be_nil
            expect(tlm.get_item("OPENC3_VERSION").bit_size).to eql(240)
            expect(tlm.read("RUBY_VERSION")).to_not be_nil
            expect(tlm.read("USER_VERSION")).to_not be_nil
            cmd = System.commands.packet("SYSTEM", "META")
            expect(cmd.read("PKTID")).to eql tlm.read("PKTID")
          ...
          it "doesn't allow COMMAND SYSTEM META definitions" do
            ...
            file.puts "APPEND_PARAMETER OPENC3_VERSION 240 STRING ''"
    ...
    describe "process_file" do
      skip "TODO: process_file doesn't exist anymore ... how much of this do we keep" do
        before(:all) do
          ...
          FileUtils.mv @config_targets, OpenC3::USERPATH
        ...
        after(:all) do
          ...
          FileUtils.mv File.join(OpenC3::USERPATH, 'targets'), File.join(OpenC3::USERPATH, 'config')
        end
    ...
    describe "OpenC3.write_exception_file" do
      it "writes a file with the exception" do
        filename = OpenC3.write_exception_file(RuntimeError.new("HELP!"))
