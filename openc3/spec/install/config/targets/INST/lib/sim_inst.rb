...
module OpenC3
  class SimInst < SimulatedTarget
    ...
    def initialize(target_name)
      ...
      position_filename = File.join(::OpenC3::USERPATH, 'config', 'data', 'position.bin')
      attitude_filename = File.join(::OpenC3::USERPATH, 'config', 'data', 'attitude.bin')
      ...
      pending_packets.each do |packet|
        ...
        when 'ADCS'
          ...
          if pos_data.nil? or pos_data.length == 0
            ...
            @position_file.close
            @position_file = File.open(File.join(::OpenC3::USERPATH, 'config', 'data', 'position.bin'), 'rb')
            pos_data = @position_file.read(44)
            @position_file_bytes_read = 44
          end
          ...
          if att_data.nil? or att_data.length == 0
            @attitude_file.close
            @attitude_file = File.open(File.join(::OpenC3::USERPATH, 'config', 'data', 'attitude.bin'), 'rb')
            att_data = @attitude_file.read(40)
            @attitude_file_bytes_read = 40
          end
