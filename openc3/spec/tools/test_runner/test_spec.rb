...
$stop_script = false
class MySuite < OpenC3::TestSuite
  def setup
    OpenC3::Test.puts "#{OpenC3::Test.current_test_suite}::#{OpenC3::Test.current_test_group}::#{OpenC3::Test.current_test_case}"
  end
...
class MechTest < OpenC3::Test
  def setup
    # current_test and current_test_group are the same
    OpenC3::Test.puts "#{OpenC3::Test.current_test_suite}::#{OpenC3::Test.current_test}::#{OpenC3::Test.current_test_case}"
  end
  ...
  def test_mech3
    OpenC3::Test.puts "#{OpenC3::Test.current_test_suite}::#{OpenC3::Test.current_test_group}::#{OpenC3::Test.current_test_case}"
    raise OpenC3::SkipTestCase, "unimplemented"
  end
      ...
      context "with run" do
        it "runs all test cases" do
          ...
          capture_io do |stdout|
            ...
            # where the OpenC3::Test.puts shows up in both stdout and the messages
      ...
      context "with run" do
        it "runs test cases in added order" do
          ...
          capture_io do |stdout|
            ...
            # Note OpenC3::Test.puts shows up in both stdout and the messages
