...
module Cosmos
  describe RouterStatusModel do
    before(:each) do
      mock_redis()
    end

    describe "self.get" do
      it "returns the specified router" do
        model = RouterStatusModel.new(name: "TEST_INT", state: "CONNECTED", scope: "DEFAULT",
          txsize: 10, rxsize: 20) # Set a few things to check
        model.create
        model = RouterStatusModel.new(name: "SPEC_INT", state: "CONNECTED", scope: "DEFAULT",
    ...
    describe "self.names" do
      it "returns all router names" do
        ...
        model = RouterStatusModel.new(name: "SPEC_INT", state: "CONNECTED", scope: "DEFAULT")
        model.create
        model = RouterStatusModel.new(name: "OTHER_INT", state: "CONNECTED", scope: "OTHER")
        model.create
        names = RouterStatusModel.names(scope: "DEFAULT")
        # contain_exactly doesn't care about ordering and neither do we
        expect(names).to contain_exactly("TEST_INT", "SPEC_INT")
        names = RouterStatusModel.names(scope: "OTHER")
        expect(names).to contain_exactly("OTHER_INT")
      end
    end

    describe "self.all" do
      it "returns all the parsed routers" do
        model = RouterStatusModel.new(name: "TEST_INT", state: "CONNECTED", scope: "DEFAULT",
          txsize: 10, rxsize: 20) # Set a few things to check
        model.create
        model = RouterStatusModel.new(name: "SPEC_INT", state: "CONNECTED", scope: "DEFAULT",
