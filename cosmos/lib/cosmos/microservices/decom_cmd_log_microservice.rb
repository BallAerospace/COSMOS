...
module Cosmos
  ...
  class DecomCmdLogMicroservice < Microservice

    METRIC_NAME = "decom_cmd_log_duration_seconds"
    ...
    def setup_plws
      ...
      @topics.each do |topic|
        ...
        remote_log_directory = "#{scope}/decomlogs/cmd/#{target_name}/#{packet_name}"
    ...
    def decom_log_data(plws, topic, msg_id, msg_hash, redis)
      ...
      plws[topic].write(:JSON_PACKET, :CMD, target_name, packet_name, msg_hash["time"].to_i, ConfigParser.handle_true_false(msg_hash["stored"]), msg_hash["json_data"], nil)
      @count += 1
      ...
      @error = err
      Logger.error("DecomCmdLog error: #{err.formatted}")
    end

  end
end

Cosmos::DecomCmdLogMicroservice.run if __FILE__ == $0
