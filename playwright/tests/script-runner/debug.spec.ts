...
test('retries failed checks', async ({ page }) => {
  ...
  await expect(page.locator('[data-test=output-messages] td:has-text("1 == 2 is FALSE")')).toHaveCount(2)
  await expect(page.locator("[data-test=state]")).toHaveValue("error");
  await page.locator('[data-test=go-button]').click()
  await expect(page.locator("[data-test=state]")).toHaveValue("stopped");
})
...
test.only('displays disconnect icon', async ({ page }) => {
  ...
  await expect(page.locator("[data-test=state]")).toHaveValue("stopped");
  await expect(page.locator('[data-test="output-messages"]')).toContainText('count1:0 count2:0')
