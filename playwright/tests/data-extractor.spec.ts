...
test('adds an entire packet', async ({ page }) => {
  ...
  // Since we're checking count() which is instant we need to poll
  await expect.poll(() => page.locator('[data-test=itemList] > div').count()).toBeGreaterThan(10)
  // Ensure we didn't add the entire packet like above
  await expect.poll(() => page.locator('[data-test=itemList] > div').count()).toBeLessThan(50)
})
...
test('add, edits, deletes items', async ({ page }) => {
  ...
  await page.locator('[data-test="itemList"] >> text=INST - ADCS - CCSDSSHF + (RAW)')
...
test('edit all items', async ({ page }) => {
  ...
  expect(await page.locator('[data-test=itemList] > div').count()).toBeGreaterThan(20)
  ...
  await page.locator('[data-test="itemList"] >> text=INST - ADCS - CCSDSSHF + (RAW)')
  await page.locator('[data-test="itemList"] >> text=INST - ADCS - POSX + (RAW)')
  await page.locator('[data-test="itemList"] >> text=INST - ADCS - VELX + (RAW)')
